...
final class DiscloseContextSigner
{
     ...
     */
    public function sign(DiscloseContext $context): array
    {
        $payload = $context->toArray();

        return [
            'd' => $this->encodePayload($payload),
            'h' => $this->checksumCalculator->calculateForArray($payload),
        ];
    }
    ...
    public function fromRequest(Request $request): DiscloseContext
    {
        ...
        if (!hash_equals($this->checksumCalculator->calculateForArray($payload), $checksum)) {
            throw new BadRequestHttpException('The disclose context has been tampered with.');
        }

        return DiscloseContext::fromArray($payload);
    }
